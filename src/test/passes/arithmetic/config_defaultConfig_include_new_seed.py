...
class MyConfig(chaos_android.ObfuscationConfig):
    chaos_android.config.probability_seed = 2026;

    def __init__(self):
        super().__init__()
    def obfuscate_arithmetic(self, mod: chaos_android.Module,
                                   fun: chaos_android.Function) -> chaos_android.ArithmeticOpt:
        return chaos_android.ArithmeticOpt(rounds=2) if chaos_android.ObfuscationConfig.default_config(self, mod, fun, [], [], [], 100) else False
