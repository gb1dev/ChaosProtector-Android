...
config.name = "ChaosProtector Android Tests"
...
if config.chaos_plugin_abi == 'Android' or \
   config.chaos_plugin_abi == 'CustomAndroid':
...
if config.chaos_plugin_abi == 'Apple':
...
plugin_file = os.path.join(config.chaos_plugin_dir, 'libChaosAndroid' + config.llvm_plugin_suffix)
print("Testing plugin file:", plugin_file)
config.substitutions.append(('%libChaosAndroid', plugin_file))

print("Available features are:", config.available_features)

# We need this to find the Python standard library
if 'CHAOS_ANDROID_PYTHONPATH' in os.environ:
    print("CHAOS_ANDROID_PYTHONPATH:", os.environ['CHAOS_ANDROID_PYTHONPATH'])
    config.environment['CHAOS_ANDROID_PYTHONPATH'] = os.environ['CHAOS_ANDROID_PYTHONPATH']
else:
    print("Please set the environment variable CHAOS_ANDROID_PYTHONPATH and try again, e.g.:")
    print("")
    print("  export CHAOS_ANDROID_PYTHONPATH=/path/to/Python-3.10.7/Lib")
    print("")
    print("For more info see: https://obfuscator.re/chaos_android/introduction/getting-started/#python-standard-library")
    exit(1)
